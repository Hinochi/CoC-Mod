...
use namespace kGAMECLASS;
	...
	public class Player extends PlayerHelper {
		...
		*/
		...
		public function fillBladder(amnt:Number = 0):void {
			var oldBladder:Number = bladder;
			bladder -= amnt;
			if (bladder < 0) bladder = 0;
			if (bladder < oldBladder && flags[kFLAGS.USE_OLD_INTERFACE] == 0) kGAMECLASS.mainView.statsView.showStatDown('bladder');
			
		}
		
		public function refillBladder(amnt:Number = 0, nl:Boolean = true):void {
			if (flags[kFLAGS.WATERSPORTS_ENABLED] > 0)
			{
				...
				bladder += amnt;
				if (bladder > 100)
				if (bladder > oldBladder && flags[kFLAGS.USE_OLD_INTERFACE] == 0) kGAMECLASS.mainView.statsView.showStatUp('bladder');
				//game.dynStats("lus", 0, "resisted", false);
				if (nl) outputText("\n");
				//Messages
				if (bladder < 10) outputText("<b>You feel fine. </b>");
				else if (bladder >= 10 && bladder < 25) outputText("<b>You feel a slight pressure, but it's not bad. </b>");
				else if (bladder >= 25 && bladder < 50) outputText("<b>A good scare would make you wet, but you've got total control otherwise. </b>");
				else if (bladder >= 50 && bladder < 75) outputText("<b>You should probably find a toilet soon. </b>");
				else if (bladder >= 75 && bladder < 90) outputText("<b>You're having trouble walking. </b>");
				else if (bladder >= 90) outputText("<b>Your bladder feels so full. </b>");
				if (bladder > oldBladder) kGAMECLASS.mainView.statsView.showStatUp("bladder");
				game.dynStats("lus", 0, "resisted", false);
				kGAMECLASS.statScreenRefresh();
			}
		}
