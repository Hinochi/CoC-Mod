	...
	public class PlayerEvents extends BaseContent implements TimeAwareInterface {
		...
		public function timeChange():Boolean {
			...
			if (flags[kFLAGS.WATERSPORTS_ENABLED] > 0) {
				...
				if (player.bladder < 80) player.bladder += (0.5 * multiplier); //Should hopefully work.
				if (player.bladder < 50) player.bladder += (0.5 * multiplier);
				if (player.bladder < 25) player.bladder += (0.5 * multiplier);
				if (player.bladder < 1) player.bladder += (0.5 * multiplier);
			}
